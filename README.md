Hardware: These programs were written for a KUKA KR6 R900 sixx Agilus, controlled by a KR C4 smallsize-2. Gripper: a forklift scoop was used, combined with 5 pallets that were held in a palletholding rack. 

Software:Files were extracted using WorkVisual6.0.

Pallets were aligned with eachother, so only the distance in Z-direction was changed.
...
Older versions: After every session, code was copied to keep a log. These older versions of the code can be found in the dated folders.

Inspection: An inspection sheet for the robot + controller combination was made for future students to perform regular inspections.
